...
nfToCFType : {auto c : Ref Ctxt Defs} ->
             FC -> NF [] -> Core CFType
nfToCFType _ (NPrimVal _ $ PrT IntType) = pure CFInt
nfToCFType _ (NPrimVal _ $ PrT IntegerType) = pure CFInteger
nfToCFType _ (NPrimVal _ $ PrT Bits8Type) = pure CFUnsigned8
nfToCFType _ (NPrimVal _ $ PrT Bits16Type) = pure CFUnsigned16
nfToCFType _ (NPrimVal _ $ PrT Bits32Type) = pure CFUnsigned32
nfToCFType _ (NPrimVal _ $ PrT Bits64Type) = pure CFUnsigned64
nfToCFType _ (NPrimVal _ $ PrT Int8Type) = pure CFInt8
nfToCFType _ (NPrimVal _ $ PrT Int16Type) = pure CFInt16
nfToCFType _ (NPrimVal _ $ PrT Int32Type) = pure CFInt32
nfToCFType _ (NPrimVal _ $ PrT Int64Type) = pure CFInt64
nfToCFType _ (NPrimVal _ $ PrT StringType) = pure CFString
nfToCFType _ (NPrimVal _ $ PrT DoubleType) = pure CFDouble
nfToCFType _ (NPrimVal _ $ PrT CharType) = pure CFChar
nfToCFType _ (NPrimVal _ $ PrT WorldType) = pure CFWorld
nfToCFType _ (NBind fc _ (Pi _ _ _ ty) sc)
    = do defs <- get Ctxt
         sty <- nfToCFType fc !(evalClosure defs ty)
         sc' <- sc defs (toClosure defaultOpts [] (Erased fc Placeholder))
         tty <- nfToCFType fc sc'
         pure (CFFun sty tty)
nfToCFType _ (NTCon fc n_in _ _ args)
    = do defs <- get Ctxt
         n <- toFullNames n_in
         case !(getNArgs defs n $ map snd args) of
              User un uargs =>
                do nargs <- traverse (evalClosure defs) uargs
                   cargs <- traverse (nfToCFType fc) nargs
                   pure (CFUser n cargs)
              Struct n fs =>
                do fs' <- traverse
                             (\ (n, ty) =>
                                    do tynf <- evalClosure defs ty
                                       tycf <- nfToCFType fc tynf
                                       pure (n, tycf)) fs
                   pure (CFStruct n fs')
              NUnit => pure CFUnit
              NPtr => pure CFPtr
              ...
              NIORes uarg =>
                do narg <- evalClosure defs uarg
                   carg <- nfToCFType fc narg
                   pure (CFIORes carg)
nfToCFType _ (NType _ _)
    = pure (CFUser (UN (Basic "Type")) [])
nfToCFType _ (NErased _ _)
...
getCFTypes args (NBind fc _ (Pi _ _ _ ty) sc)
    = do defs <- get Ctxt
         aty <- nfToCFType fc !(evalClosure defs ty)
         sc' <- sc defs (toClosure defaultOpts [] (Erased fc Placeholder))
         getCFTypes (aty :: args) sc'
getCFTypes args t
    = pure (reverse args, !(nfToCFType (getLoc t) t))
